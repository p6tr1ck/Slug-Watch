...
function parseThruDesc(w, details) {
  ...
  const rules = [
    {
      keyword: ["last name", "address", "phone number", "photos", "name"],
      weight: 3,
    }, //doxx,
    {
      keyword: [
  ...
  return (
    ...
    >
      ...
      <div className="px-3 pb-3 pt-2 flex items-center justify-between gap-2">
        <button
          className="px-3 py-1.5 rounded-lg border border-slate-300 text-sm hover:bg-slate-50 cursor-pointer"
          onClick={onCancel}
        >
          Back
        </button>
        <button
          className="px-3 py-1.5 rounded-lg bg-rose-600 text-white text-sm hover:bg-rose-700 cursor-pointer"
          onClick={submit}
        >
          Submit
        </button>
